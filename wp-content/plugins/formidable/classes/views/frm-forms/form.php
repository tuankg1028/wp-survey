...
<div id="frm_form_editor_container">
    ...
    <div class="postbox">
        <div id="titlediv" class="inside">
            <input type="text" name="name" value="<?php echo esc_attr($form->name); ?>" id="title"
                   placeholder="<?php esc_attr_e('Enter title here', 'formidable'); ?>"/>
        </div>
        <div class="inside">
            <select name="category" id="" style="width: 100%;">
                <option value="0">Niên khóa</option>
                <?php foreach ($categories as $category) : ?>
                    <option value="<?= $category->term_id ?>" <?= $form->category == $category->term_id ? 'selected' : '' ?> ><?= $category->name ?></option>
                <?php endforeach; ?>
            </select>
        </div>
